...
const ApplicationModal: React.FC<Props> = ({ positionId, onClose }) => {
  const { user } = useAuth()
  const { submit, isLoading, error } = useApplications()
  const [submitted, setSubmitted] = useState(false)

  const handleSubmit = async () => {
    if (!user) return
    await submit({ student_id: user.id, position_id: positionId })
      ...
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        {submitted ? (
          ...
          <>
            ...
            <p style={styles.text}>
              You are applying for position <strong>{positionId.slice(0, 8)}…</strong>
            </p>
